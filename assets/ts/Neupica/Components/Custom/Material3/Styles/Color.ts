...
export var hexToRgba = function(hex) {
    ...
    hex = hex.replace('#', '');
    if (3 === hex.length) {
        r = hex.charAt(0);g = hex.charAt(1);b = hex.charAt(2);
    } else if (4 === hex.length) {
        r = hex.charAt(0);g = hex.charAt(1);b = hex.charAt(2);a = hex.charAt(3);
    } else if (6 === hex.length) {
        r = hex.substring(0, 2);g = hex.substring(2, 4);b = hex.substring(4, 6);
    } else if (8 === hex.length) {
        r = hex.substring(0, 2);g = hex.substring(2, 4);b = hex.substring(4, 6);a = hex.substring(6, 8);
    } else {
        return '';
    }
...
export function setStorageDark(light: string) {
    storageDark = light
    if (storageDark == 'dark') {
        colorScheme = theme.schemes.dark
    } else {
